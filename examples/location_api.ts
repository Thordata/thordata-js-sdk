...
async function main() {
  ...
  // List countries (proxy_type: 1 = residential, 2 = unlimited)
  console.log("1. Listing countries...");
  const countries = await client.listCountries(1);
  ...
  // List states for US
  console.log("\n2. Listing US states...");
  const states = await client.listStates("US", 1);
  ...
  const cities = await client.listCities("US", "california", 1);
  ...
  // List ASNs for US
  console.log("\n4. Listing US ASNs...");
  const asns = await client.listAsns("US", 1);
